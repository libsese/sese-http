...
void HttpConnectionImpl::asyncReadSome(const asio::mutable_buffers_1 &buffer, const std::function<void(const asio::error_code &error, std::size_t bytes_transferred)> &callback) {
...
HttpsConnectionImpl::HttpsConnectionImpl(
    ...
    remote_address = sese::internal::net::convert(this->stream->next_layer().remote_endpoint());
}
...
void HttpsConnectionImpl::asyncReadSome(const asio::mutable_buffers_1 &buffer, const std::function<void(const asio::error_code &error, std::size_t bytes_transferred)> &callback) {
