...
void HttpConnectionExImpl::writeBlocks(const std::vector<asio::const_buffer> &buffers, const std::function<void(const asio::error_code &code)> &callback) {
    is_write = true;
...
void HttpConnectionExImpl::writeBlock(const void *buffer, size_t size, const std::function<void(const asio::error_code &code)> &callback) {
    is_write = true;
...
void HttpConnectionExImpl::checkKeepalive() {
    // if (keepalive) {
...
void HttpsConnectionExImpl::writeBlocks(const std::vector<asio::const_buffer> &buffers, const std::function<void(const asio::error_code &code)> &callback) {
    is_write = true;
...
void HttpsConnectionExImpl::writeBlock(const void *buffer, size_t size, const std::function<void(const asio::error_code &code)> &callback) {
    is_write = true;
...
void HttpsConnectionExImpl::checkKeepalive() {
    // if (keepalive) {
    timer.async_wait([conn = getPtr()](const asio::error_code &error) {
        if (error == asio::error::operation_aborted) {
        } else {
